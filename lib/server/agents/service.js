...
const ensurePluginAllowed = ({ cfg, pluginKey }) => {
  ...
  cfg.plugins.entries[pluginKey] = {
    ...(cfg.plugins.entries[pluginKey] && typeof cfg.plugins.entries[pluginKey] === "object"
      ? cfg.plugins.entries[pluginKey]
      : {}),
    enabled: true,
  };
...
const normalizeChannelConfig = ({ provider, channelConfig }) => {
  ...
  for (const [key, value] of Object.entries(nextConfig)) {
    if (key === "enabled" || key === "accounts" || key === "defaultAccount") continue;
    defaultAccountConfig[key] = cloneJson(value);
    delete nextConfig[key];
  }

  const tokenField = kChannelTokenFields[normalizedProvider];
  if (Object.keys(defaultAccountConfig).length > 0 || defaultAccountConfig[tokenField]) {
...
const appendBindingToConfig = ({ cfg, agentId, match }) => {
  ...
  if (conflictingBinding) {
    ...
    throw new Error(`Binding already assigned to agent "${conflictingAgentId}"`);
  }
  const nextBinding = {
...
const hasLegacyDefaultChannelAccount = ({ config }) =>
  Object.keys(config || {}).some(
    (entry) => entry !== "accounts" && entry !== "defaultAccount" && entry !== "enabled",
  );
...
const readPairedCountsByAccount = ({ fsImpl, OPENCLAW_DIR, channelId, accountIds, config }) => {
  ...
  try {
    const files = fsImpl
      ...
      .filter(
        (fileName) =>
          String(fileName || "").startsWith(`${String(channelId || "").trim()}-`)
          && String(fileName || "").endsWith("-allowFrom.json"),
      );
    for (const fileName of files) {
      const accountId = resolveCredentialPairingAccountId({ channelId, fileName });
      if (!accountId || !counts.has(accountId)) continue;
      const filePath = path.join(credentialsDir, fileName);
      const parsed = JSON.parse(fsImpl.readFileSync(filePath, "utf8"));
      const pairedCount = Array.isArray(parsed?.allowFrom) ? parsed.allowFrom.length : 0;
  ...
  for (const accountId of counts.keys()) {
    const accountConfig =
      accountId === "default" && !(config.accounts && typeof config.accounts === "object")
        ? config
        : config.accounts?.[accountId] || {};
    const inlineAllowFrom = accountConfig?.allowFrom;
    if (!Array.isArray(inlineAllowFrom)) continue;
    counts.set(accountId, Number(counts.get(accountId) || 0) + inlineAllowFrom.length);
  }
...
const listConfiguredChannelAccounts = ({ fsImpl, OPENCLAW_DIR, cfg }) => {
  ...
  return Object.entries(channels)
    .map(([channelId, channelConfig]) => {
      const config = channelConfig && typeof channelConfig === "object" ? channelConfig : {};
      const accountsConfig =
        config.accounts && typeof config.accounts === "object" ? config.accounts : {};
        ...
        accounts: normalizedAccountIds
          .map((accountId) => {
            ...
            return {
              ...
              boundAgentId:
                boundAccountMap.get(`${String(channelId || "").trim()}:${accountId}`) || "",
              paired: Number(pairedCounts.get(accountId) || 0),
              status: Number(pairedCounts.get(accountId) || 0) > 0 ? "paired" : "configured",
            };
          })
          .sort((left, right) => {
...
const calculatePathSizeBytes = ({ fsImpl, targetPath }) => {
  ...
  if (!stat) return 0;
  if (typeof stat.isSymbolicLink === "function" && stat.isSymbolicLink()) return 0;
...
const createAgentsService = ({
  ...
  clawCmd = async () => ({ ok: false, stdout: "", stderr: "openclaw command unavailable" }),
}) => {
  ...
  const getAgentWorkspaceSize = (agentId) => {
    ...
    const workspacePath = String(
      agent.workspace || resolveAgentWorkspacePath({ OPENCLAW_DIR, agentId: normalized }),
    ).trim();
    if (!workspacePath) {
  ...
  const addBinding = (agentId, input = {}) => {
    ...
    const agent = cfg.agents.list.find((entry) => entry.id === normalizedAgentId);
  ...
  const createChannelAccount = async (input = {}) => {
    ...
    const existingAccounts =
      normalizedChannelConfig.accounts && typeof normalizedChannelConfig.accounts === "object"
        ? normalizedChannelConfig.accounts
        : {};
    const requestedAccountId = String(input.accountId || "").trim();
    const accountId = requestedAccountId || (Object.keys(existingAccounts).length > 0 ? "" : "default");
    if (!accountId) {
      throw new Error("Channel account id is required");
    }
    if (!isValidChannelAccountId(accountId)) {
      throw new Error("Channel account id must be lowercase letters, numbers, and hyphens only");
    }
    if (existingAccounts[accountId]) {
      throw new Error(`Channel account "${provider}/${accountId}" already exists`);
    }
    ...
    if (duplicateEnvEntry) {
      ...
      console.log(`[alphaclaw] Overwriting orphaned channel env var ${dupKey} (no matching config entry)`);
    }
    ...
    try {
      ...
      const addResult = await clawCmd(addArgs.join(" "), { quiet: true, timeoutMs: 30000 });
      if (!addResult?.ok) {
        throw new Error(addResult?.stderr || addResult?.stdout || "Could not add channel account");
      }
      ...
      const nextProviderConfig = normalizeChannelConfig({
        provider,
        channelConfig:
          nextCfg.channels?.[provider] && typeof nextCfg.channels[provider] === "object"
            ? nextCfg.channels[provider]
            : {},
      });
      const nextAccounts =
        nextProviderConfig.accounts && typeof nextProviderConfig.accounts === "object"
          ? { ...nextProviderConfig.accounts }
          : {};
      nextAccounts[accountId] = {
        ...(nextAccounts[accountId] && typeof nextAccounts[accountId] === "object"
          ? nextAccounts[accountId]
          : {}),
        ...(name ? { name } : {}),
        [tokenField]: `\${${envKey}}`,
      ...
      nextCfg.channels = nextCfg.channels && typeof nextCfg.channels === "object"
        ? { ...nextCfg.channels }
        : {};
      ...
      if (!bindResult?.ok) {
        throw new Error(bindResult?.stderr || bindResult?.stdout || "Could not bind channel account");
      }
    } catch (error) {
      try {
        await clawCmd(
  ...
  const updateChannelAccount = (input = {}) => {
    ...
    const hasAccounts =
      providerConfig.accounts && typeof providerConfig.accounts === "object";
    const hasLegacyDefault =
      accountId === "default" && !hasAccounts && hasLegacyDefaultChannelAccount({ config: providerConfig });
    if (!hasLegacyDefault && !providerConfig.accounts?.[accountId]) {
      throw new Error(`Channel account "${provider}/${accountId}" not found`);
    }
  ...
  const cleanupChannelAccountPairingFiles = ({ provider, accountId }) => {
    ...
    const allowFromPatterns = [
      ...
      ...(normalizedAccountId === "default" ? [`${provider}-allowFrom.json`] : []),
    ];
  ...
  const deleteChannelAccount = async (input = {}) => {
    ...
    if (provider === "discord") {
      ...
      const targetMatch = normalizeBindingMatch({ channel: provider, accountId });
      const existingBindings = Array.isArray(nextCfg.bindings) ? nextCfg.bindings : [];
    ...
    const removeResult = await clawCmd(removeArgs.join(" "), { quiet: true, timeoutMs: 30000 });
    if (!removeResult?.ok) {
      throw new Error(removeResult?.stderr || removeResult?.stdout || "Could not delete channel account");
    }

...
export const useBrowseNavigation = ({
  ...
  useEffect(() => {
    setSidebarTab((currentTab) => {
      if (location.startsWith("/browse")) return "browse";
      if (location.startsWith("/agents")) return "agents";
      if (currentTab === "browse" || currentTab === "agents") return "menu";
      return currentTab;
    });
  }, [location]);
  ...
  useEffect(() => {
    if (location.startsWith("/browse")) return;
    if (location.startsWith("/agents")) return;
  ...
  const [lastAgentsRoute, setLastAgentsRoute] = useState("/agents");

  useEffect(() => {
    if (!location.startsWith("/agents")) return;
    setLastAgentsRoute((current) =>
  ...
  const handleSelectSidebarTab = useCallback((nextTab) => {
    ...
    if (nextTab === "menu" && (location.startsWith("/browse") || location.startsWith("/agents"))) {
      setBrowsePreviewPath("");
    ...
    if (nextTab === "agents" && !location.startsWith("/agents")) {
      setLocation(lastAgentsRoute || "/agents");
    }
  }, [lastAgentsRoute, lastBrowsePath, lastMenuRoute, location, setLocation]);

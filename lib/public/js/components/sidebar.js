...
import { HomeLineIcon, FolderLineIcon, RobotLineIcon } from "./icons.js";
...
export const AppSidebar = ({
  ...
  return html`
    ...
    <div class="sidebar-tabs">
      ...
      <button
        class=${`sidebar-tab ${sidebarTab === "agents" ? "active" : ""}`}
        aria-label="Agents tab"
        title="Agents"
        onclick=${() => onSelectSidebarTab("agents")}
      >
        <${RobotLineIcon} className="sidebar-tab-icon" />
      </button>
    </div>
    <div
      style=${{
    ...
    <div
      style=${{
        display: sidebarTab === "agents" ? "flex" : "none",
        flexDirection: "column",
        flex: "1 1 auto",
        minHeight: 0,
        overflow: "hidden",
        ...
        ${agents.map(
          (agent) => html`
            ...
            >
              ...
              ${agent.default && html`<${Badge} tone="cyan" style=${{ fontSize: "10px", padding: "1px 6px" }}>Default</${Badge}>`}
            </button>
          `,
        )}
        <div class="sidebar-agents-add">
          <${ActionButton}
            onClick=${onAddAgent}
            tone="secondary"
            size="sm"

...
import { fetchModels, fetchModelsConfig } from "../../lib/api.js";
...
export const CreateAgentModal = ({
  ...
  useEffect(() => {
    ...
    Promise.all([fetchModels(), fetchModelsConfig()]).then(
      ([catalogResult, configResult]) => {
        setModelCatalog(
          Array.isArray(catalogResult?.models) ? catalogResult.models : [],
        );
        setDefaultPrimary(configResult?.primary || "");
      },
    ).catch(() => {});
  }, [visible]);
  ...
  const submit = async () => {
    ...
    if (selectedModel) {
      payload.model = { primary: selectedModel };
    }
    await onSubmit(payload);
  };

  const defaultLabel = defaultPrimary || "global default";
  ...
  return html`
    ...
    >
      ...
      <div class="space-y-3">
        ...
        <label class="block space-y-1">
          <span class="text-xs text-gray-400">Primary model</span>
          <select
            value=${selectedModel}
            onInput=${(event) => setSelectedModel(event.target.value)}
            class="w-full bg-black/30 border border-border rounded-lg px-3 py-2 text-sm text-gray-200 outline-none focus:border-gray-500"
          >

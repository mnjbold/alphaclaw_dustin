...
const App = () => {
  ...
  const navigateToBrowseFile = (relativePath, options = {}) => {
    ...
    setBrowsePreviewPath((currentPreviewPath) => {
      if (!shouldPreservePreview) return "";
      const activePath = normalizeBrowsePath(
        currentPreviewPath || selectedBrowsePath || "",
      );
      if (!activePath || activePath === normalizedTargetPath) return "";
      return activePath;
    });
